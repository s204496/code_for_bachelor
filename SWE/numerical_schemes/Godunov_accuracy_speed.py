...
def main(terminal_arguments):
    ...
    cells_list = [100*2**i for i in range(5)]
    ...
    for cells in cells_list:
        speed_measurings = [] 
        U,W = None, None # just for scoping
        for j in range(avg_over):
            ...
            speed_measurings.append(elapsed_time)
        speed_list[0].append(np.mean(np.array(speed_measurings)))
        speed_list[1].append(np.std(np.array(speed_measurings)))
        ...
        error =  error_calculation.norm_1_FVM(exact_np[:, 0:-1], W_np[:, 1:-1], cells)
