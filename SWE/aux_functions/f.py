...
def get_dry_speeds(u_l, a_l, u_r, a_r):
    # 1: dry/wet front going right, end of a rarefaction going right, dry/wet front going left, end of a rarefaction going left
    return (u_r - 2*a_r, u_r + a_r, u_l + 2*a_l, u_l - a_l) 

# Get the speed of the dry/wet waves all four cases
def get_dry_speeds_np(u_l, a_l, u_r, a_r):
    # 1: dry/wet front going right, end of a rarefaction going right, dry/wet front going left, end of a rarefaction going left
    first = np.array([u_l + 2*a_l, u_l - a_l]).flatten()
    second = np.array([u_r - 2*a_r, u_r + a_r]).flatten()
