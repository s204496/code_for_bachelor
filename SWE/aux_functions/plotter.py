...
def make_plot(out_plot_name, out_path, x_center, x_interval, t_end, tuple_bool_exact_scatter, exact_data, scheme, numerical_data, figuare, ax, riemann_str, y_label, plot_title, save_name, data_driven):
    ...
    if scheme == 0: # godunov upwind method
        if data_driven == 1:
            figuare.suptitle(plot_title + ", at t = " + str(t_end) + ", " + out_plot_name + " using Godunov upwind, CNN on " + riemann_str + " Riemann Data", y = 0.93, fontsize=9.5)
        elif data_driven == 2:
            figuare.suptitle(plot_title + ", at t = " + str(t_end) + ", " + out_plot_name + " using Godunov upwind, FFNN on " + riemann_str + " Riemann Data", y = 0.93, fontsize=9.5)
        else:
    ...
    if scheme == 2: # WAF scheme 
        if data_driven:
            figuare.suptitle(plot_title + ", at t = " + str(t_end) + ", " + out_plot_name + " using WAF, Data-driven", y = 0.93, fontsize=9.5)
        else:
