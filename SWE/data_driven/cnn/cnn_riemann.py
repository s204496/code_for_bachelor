...
class cnn_riemann(nn.Module):
    def __init__(self):
        super(cnn_riemann, self).__init__()

        # Convolutional layers
        self.conv1 = nn.Conv1d(3, 20, kernel_size=2, stride=1)
...
def main(argv):
    ...
    patience = 10
    # Create data loaders
    train_loader, val_loader, test_loader = general_aux.create_data_loaders_from_csv('data_driven/generated_data/riemann_' + argv[1] + '.csv', batch_size, 'cnn_riemann')
    train(train_loader, val_loader, learning_rates, device, batch_size, maximum_epochs, patience=patience, model_path=('data_driven/models/riemann_' + argv[1] + '_CNN.pt'))
    model = general_aux.load_model(model_path=('data_driven/models/riemann_' + argv[1] + '_CNN.pt'), device=device, model_type = 'cnn_riemann')
    general_aux.test(model, test_loader, device)

if __name__ == '__main__':
    main(sys.argv)
    print("Completed training of CNN for " + sys.argv[1] + " Riemann solver")