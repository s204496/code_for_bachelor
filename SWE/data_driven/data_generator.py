...
def dt_and_outdata(scheme, solver, tolerance, iterations, dx, cfl, g, W, U):
    match scheme:
        ...
        case 2:
            return tvd_waf.single_sample(solver, tolerance, iterations, dx, cfl, g, W, U)
...
def main(argv):
    ...
    try:
        ...
        sys.exit(1)
    try:
        scheme = int(argv[2]) # 0 = lax_friedrich 1 = godunov, 2 = tvd_waf
        if (scheme > 2 or scheme < 0):
    ...
    if (scheme == 1 or scheme == 2):
        try:
            if ('exact' == argv[3]):
                ...
                print("Please specify the Riemann solver to use as third argument \'exact\' or \'hllc\'")
                sys.exit(1)
        except:
            print("Please specify the Riemann solver to use as third argument \'exact\' or \'hllc\'")
            sys.exit(1)
    ...
    if scheme == 0:
        print('Generating ' + str(samples) + ' samples for Lax Friedrich scheme ...')
    elif scheme == 1:
        print('Generating ' + str(samples) + ' samples for Godunov upwind scheme, solver ' + argv[3] + ' ...')
    else: 
        print('Generating ' + str(samples) + ' samples for TVD WAF scheme, solver ' + argv[3] + ' ...')
    ...
    if (scheme == 2):
        df = pd.DataFrame(columns=['h_ll, h_l', 'h_c', 'h_r', 'h_rr', 'u_ll', 'u_l', 'u_c', 'u_r', 'u_rr', 
                               ...
                               'delta_x', 'dt', 'CFL', 'h_out', 'hu_out', 'hpsi_out'])
    else:
    ...
    for i in range(samples):
        ...
        dx = x_len/cells
        psi_split = 1+np.random.randint(2, size=1).item()
        ...
        if (scheme == 2):
            ...
            U = np.empty((7,3), dtype=float)
        else:
            h_and_u = np.random.random(size=6)
            psi = np.empty(3, dtype=float)
            psi[0:psi_split], psi[psi_split:] = psi_l, psi_r 
            W = np.empty((5,3), dtype=float)
            W_pre = np.array([[h_and_u[0]*10, h_and_u[1]*10, psi[0]], [h_and_u[2]*10, h_and_u[3]*10, psi[1]], [h_and_u[4]*10, h_and_u[5]*10, psi[2]]])    
            U = np.empty((5,3), dtype=float)
        W[1:-1] = W_pre
        W[0], W[-1] = W_pre[0], W_pre[-1]
        U[:,0], U[:,1], U[:,2] = W[:,0], W[:,0]*W[:,1], W[:,0]*W[:,2]
        (dt, U_out) = dt_and_outdata(scheme, solver, tolerance, iterations, dx, cfl, g, W, U) 

        if (scheme == 2):
            df.loc[i] = np.array([h_ll, h_l, h_c, h_r, h_rr, u_ll, u_l, u_c, u_r, u_rr, 
                               psi_ll, psi_l, psi_c, psi_r,spi_rr, hu_ll, hu_l, hu_c, hu_r, hu_rr, 
                               hpsi_ll, hpsi_l, hpsi_c, hpsi_r, hspi_rr,
                               delta_x, dt, CFL, U_out[0], U_out[1], U_out[2]])
        else:
            df.loc[i] = np.array([h_l, h_c, h_r, u_l, u_c, u_r, psi_l, psi_c, psi_r, hu_l, hu_c, hu_r, hpsi_l, hpsi_c, hpsi_r, delta_x, dt, cfl, h_out, hu_out, hpsi_out])
