...
class ffnn_lax_flux(nn.Module):
    def __init__(self):
        super(ffnn_lax_flux, self).__init__()
...
def main(argv):
    ...
    train_loader, val_loader, test_loader = general_aux.create_data_loaders_from_csv('data_driven/generated_data/lax_friedrich_flux.csv', batch_size, 'ffnn_flux')
    train(train_loader, val_loader, learning_rates, device, batch_size, maximum_epochs, patience=patience, model_path=('data_driven/models/lax_flux_FFNN.pt'))
    model = general_aux.load_model(model_path=('data_driven/models/lax_flux_FFNN.pt'), device=device, model_type='ffnn_flux')
    general_aux.test(model, test_loader, device)

if __name__ == '__main__':
    main(sys.argv)
    print("Completed training of FFNN-Lax-Friedrich-flux")